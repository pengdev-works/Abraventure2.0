    ...
    const fetchOSRMRoute = async () => {
      ...
      try {
        ...
        if (routeData.code === 'Ok' && routeData.routes && routeData.routes.length > 0) {
          ...
          console.warn('No OSRM driving route found, falling back to straight line.');
    ...
    <div className="relative flex h-[calc(100vh-64px)] w-full overflow-hidden bg-slate-50">
      ...
      <div className={`fixed inset-y-0 left-0 z-[9999] w-[85vw] max-w-[360px] flex flex-col bg-white border-r border-slate-200/80 shadow-2xl transition-transform duration-300 md:relative md:translate-x-0 md:w-96 md:flex ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {selectedMunicipality && (
          <div className="bg-emerald-50 border-b border-emerald-100 p-3.5 flex flex-col gap-2 flex-shrink-0">
            ...
            {selectedMunicipality.id && (
              <Link
                ...
                className="inline-flex items-center justify-center gap-1 px-3 py-2 bg-white hover:bg-slate-50 border border-emerald-200/80 rounded-xl text-[10px] font-extrabold text-emerald-900 shadow-sm transition-all"
              >
        ...
        >
          ...
          {routeLine && (
            <>
              ...
              <Polyline
                positions={routeLine}
                color={routeSummary?.isFallback ? "#8b5cf6" : "#4f46e5"} // Purple for fallback, Indigo for route
                weight={5}
                opacity={0.8}
                lineCap="round"
                dashArray={routeSummary?.isFallback ? "10, 10" : undefined}
              />
              {/* Inner glowing core line */}
              <Polyline
                positions={routeLine}
                color={routeSummary?.isFallback ? "#a78bfa" : "#6366f1"}
                weight={2}
                opacity={0.9}
                lineCap="round"
                dashArray={routeSummary?.isFallback ? "10, 10" : undefined}
